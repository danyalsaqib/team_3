...
class Planner:
    ...
    def map_callback(self, msg):
        ...
        if np.abs(angle) > 0.1 and self.initial_turn == False and norm > 0.5:
            ...
            #self.cmd.angular.z = angle * 0.4
            
            # if angle > 1:
            #     print("modded_turner: ", angle * 0.2)
            #     self.cmd.angular.z = angle * 0.2

            # else:
            #     print("modded_turner: ", 0.1)
            #     self.cmd.angular.z = (angle / np.abs(angle)) * 0.09
            ...
            #norm_pos_des = np.linalg.norm(pos_des[:2])
            #angle_modded = angle_modded * 0.15
            if angle_modded > 1:
                print("modded_turner: ", angle_modded * 0.2)
                self.cmd.angular.z = angle_modded * 0.2

            else:
                print("modded_turner: ", 0.1)
                self.cmd.angular.z = (angle_modded / np.abs(angle_modded)) * 0.1
